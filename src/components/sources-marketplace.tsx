  ...
  return <>
    ...
    <Card className="overflow-hidden">
      ...
      <div className="table-wrap"><table><thead><tr><th>Source</th><th>Creator</th><th>Price</th><th>Citations</th><th>Earned</th><th /></tr></thead><tbody>{sources.map((source) => <tr key={source.id}>
        <td className="min-w-80"><p className="font-semibold text-slate-750">{source.title}</p><p className="mt-1 line-clamp-1 max-w-md text-xs text-slate-400">{source.excerpt}</p><div className="mt-2 flex flex-wrap gap-1">{source.tags.split(",").slice(0, 3).map((tag) => <Badge key={tag}>{tag.trim()}</Badge>)}</div></td>
