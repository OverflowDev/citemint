  ...
  return <>
    ...
    <Card className="overflow-hidden">
      ...
      <div className="table-wrap"><table><thead><tr><th>Source</th><th>Creator</th><th>Price</th><th>Citations</th><th>Earned</th><th /></tr></thead><tbody>{sources.map((source) => <tr key={source.id}>
        ...
        <td><p className="font-semibold text-slate-650">{source.creator.name}</p><p className="mt-1 font-mono text-[10px] text-slate-400">{shortWallet(source.creator.walletAddress)}</p>{source.creator.walletVerifiedAt && <div className="mt-2"><Badge tone="green">{source.creator.walletVerificationMethod === "eip191" ? "wallet verified" : "test wallet"}</Badge></div>}</td>
