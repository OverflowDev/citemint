  ...
  return <>
    ...
    <div className="grid gap-5 xl:grid-cols-[.82fr_1.18fr]">
      ...
      <div>
        ...
        {!result && <Card className="grid min-h-140 place-items-center border-dashed p-8 text-center"><div className="max-w-sm"><div className="mx-auto grid size-14 place-items-center rounded-2xl bg-[#eef5f1] text-[#39745e]">{loading ? <LoaderCircle size={23} className="animate-spin" /> : <Sparkles size={23} />}</div><p className="mt-5 text-lg font-semibold text-slate-700">{loading ? "Agent is settling evidence" : "Fund the escrow, then ask"}</p><p className="mt-2 text-sm leading-6 text-slate-400">Every successful citation will include an Arc transaction hash that anyone can verify.</p></div></Card>}
